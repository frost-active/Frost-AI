...
def convert_to_device_schema(plan):
    ...
    for t in plan.get("tasks", []):
        ...
        def apply(task_key, interval_key, default_interval):
            ...
            if t.get("mode") == "absolute" and t.get("times"):
                ...
                config[task_key].update({
                    ...
                    "mode": "interval",
                    interval_key:
                        (t.get("interval_minutes") or default_interval)
                        * (60000 if "ms" in interval_key else 1),
        ...
        # =========================
            ...
            # ✅ NO explicit timing
            # → use lap mode
            if not t.get("start_time") and not t.get("end_time"):

                config["pomo"]["lap_mode_enabled"] = True
            ...
            # → disable lap mode
                ...
                config["pomo"]["lap_mode_enabled"] = False

                # IMPORTANT:
                # explicit schedule -> no laps
                config["pomo"]["laps"] = []
...
def parse_schedule():
    ...
    try:
        ...
        logs.append(
            f"⏱ Total time: {round(elapsed, 2)} ms"
        )
